...
namespace TransactionalNodeService.SharePoint
{
    public class WebAppSPGlymaSession : IGlymaSession
    {
        ...
        private SPWeb _elevatedWeb = null;
        private SPSite _elevatedSite = null;
        ...
        private SPSite ElevatedSite
        {
            get
            {
                if (_elevatedSite == null)
                {
                    SPSecurity.RunWithElevatedPrivileges(delegate()
                    {
                        _elevatedSite = new SPSite(_callingUrl);
                    });
                }
        ...
        private void FindSecurityDbConnectionParameters(ref GlymaSessionConfiguration glymaSessionConfig)
        {
            ...
            while (!isExhausted)
            {
                try
                {
                    if (web.Properties.ContainsKey("Glyma.SecurableContextId") && web.Properties.ContainsKey("Glyma.SecurityDatabaseServer") && web.Properties.ContainsKey("Glyma.SecurityDatabaseName"))
                    {
                        ...
                        glymaSessionConfig.SecurableContextId = securableContextId;
                        isExhausted = true;
                        break;
                    }
                    else
            ...
            if (securableContextId == -1 || string.IsNullOrEmpty(securityDatabaseServer) || string.IsNullOrEmpty(securityDatabaseName))
            {
                throw new NullReferenceException("No securable context ID, security database server, or security database name provided! Please assign the Glyma.SecurableContextId property, Glyma.SecurityDatabaseServer property, and the Glyma.SecurityDatabaseName property on the containing SPWeb.");
            }
        }
